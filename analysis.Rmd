...
m.long.pandemic <- reshape2::melt(m_pandemic) %>%
  rename('initial_age'=Var1, index=Var2, uncond_prob=value) %>%
  filter(index >= initial_age) %>%
  arrange(initial_age, index) %>%
  group_by(initial_age) %>%
  ...
  ungroup() %>%
  select(N_deaths_expected_Post_pand, age_death, yearN) %>%
  left_join(m.long, by=c('age_death','yearN'))

m.long.pandemic %>% 
  filter(age_death==65 & yearN<=10) %>%
ggplot( aes(x=yearN, y=N_deaths_expected_Post_pand, group=age_death, color='red')) +
  geom_line()+
  geom_line(aes(x=yearN, y=N_deaths_expected, col='gray')) +
  theme_classic() +
  ylim(0,NA)
