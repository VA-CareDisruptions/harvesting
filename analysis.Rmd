...
```{r}
#https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0130137

# formula
# formula1 <- N_deaths ~  1 + #intercept
#     t +   #fixed effect for time
#   pandemic_period + #fixed effect for pandemic
#     f(agey,model = "ar1", hyper=list(prec = list(prior = "loggamma", param = log(c(3,2)))))+  #age-specific intercept
#     f(agey2,t,model = "ar1", hyper=list(prec = list(prior = "loggamma", param = log(c(3,2))))) +   #age-varying effect for time
#     f(agey3,pandemic_period,model = "ar1", hyper=list(prec = list(prior = "loggamma", param = log(c(3,2)))))+  #age-varying effect for pandemic
#    log.offset
...
p2 <- ggplot(re.age, aes(x=ID, y=mean))+
  geom_line()+
  theme_classic()+
...
p3 <- ggplot(re.age.time, aes(x=ID, y=mean))+
  geom_line()+
  theme_classic()+
  geom_ribbon(data=re.age.time, aes(x=ID, ymin=`0.025quant`, ymax=`0.975quant`), alpha=0.2)
p3

re.age.pandemic <- inla.out$summary.random$agey3 %>%
  mutate()

p4 <- ggplot(re.age.pandemic, aes(x=ID, y=mean))+
  geom_line()+
  theme_classic()+
  geom_ribbon(data=re.age.pandemic, aes(x=ID, ymin=`0.025quant`, ymax=`0.975quant`), alpha=0.2)
p4

plot(inla.out$summary.random$agey$mean, type='l')

plot(inla.out$summary.random$agey2$mean, type='l')

plot(inla.out$summary.random$agey3$mean, type='l')
