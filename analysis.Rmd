...
# all_death  <-  open_dataset("R:/NCHS_mortality_public/parquet", format = "parquet") %>%
#   group_by(agey, year, month) %>%
#   summarize(N_deaths=n()) %>%
#   collect() 
# saveRDS(all_death,'./Data/all_deaths_single_year_age.rds')



#wonder
w1 <- read.csv('./Data/WONDER_provisional_2023_04_23.txt', header=T, sep="\t") %>%
  ...
  rename(agey= Single.Year.Ages.Code, N_deaths_w=Deaths) %>%
  filter(date<'2023-04-01' & agey !='NS') %>%
  select(agey, N_deaths_w, year, month, date)


a1 <- readRDS('./Data/all_deaths_single_year_age.rds')
#Uses age recode 27
#https://www.cdc.gov/nchs/data/dvs/Multiple-Cause-Record-Layout-2020.pdf 


a2 <- a1 %>%
  group_by(agey, month, year) %>%
  mutate(agey=floor(agey),
          agey = if_else(agey>100,100, agey)) %>%
  filter(agey !=999) %>%
  summarize(N_deaths=sum(N_deaths)) %>%
  ungroup() %>%
    tidyr::complete(agey, month, year, fill=list(N_deaths=0)) %>% #fills 0s
  ...
  full_join(w1, by=c('agey', 'month', 'year','date')) %>%
  mutate( N_deaths = if_else(year>=2022, N_deaths_w, N_deaths)) #fill in with wonder data for 2022, 2023
...
a3 <- a2 %>%
  left_join(pop2, by=c('agey','date')) %>%
  mutate( pandemic_period = if_else(date>='01-01-2020',1,0)) %>% #since analyzig quarterly, start pandemic in Q1
  filter(!is.na(pop.interpol) & date <= '2022-06-01')
...
b1 <- a3 %>%
  ...
  mutate(log.offset=log(pop/100000), 
         ...
         index=row_number(),
         pandemic_start_index = max(ifelse(year==2020 & qtr==1, index, 0)),
         pandemic_t =  if_else(index>=pandemic_start_index, index-pandemic_start_index +1 , 0)     ) %>% 
  group_by(agey) %>%
...
b1.pred <- b1 %>%
  cbind.data.frame(inla.out$summary.fitted.values) %>%
  filter(t==1)

p0.fitted <- ggplot(b1.pred, aes(x=agey, y=log(mean/pop), group=t, col=t))+
  geom_line()+
    geom_ribbon(data=b1.pred, aes(x=agey, ymin=log(`0.025quant`/pop), ymax=log(`0.975quant`/pop)), alpha=0.2) +
  theme_classic()
...
p3.t.inla <- ggplot(re.t, aes(x=ID, y=exp(mean)))+
  geom_line()+
  theme_classic()+
  #geom_ribbon(data=re.t, aes(x=ID, ymin=exp(`0.025quant`), ymax=exp(`0.975quant`)), alpha=0.2)+
  geom_hline(yintercept = 1)

p3.t.inla

re.age.t <- inla.out$summary.random$t %>%
  mutate() %>%
  arrange(ID) %>%
  left_join(b1, by=c('ID'='id'))

p3.t.age.inla <- ggplot(re.age.t, aes(x=t, y=exp(mean), group=agey , col=agey))+
  geom_line()+
  theme_classic()+
  #geom_ribbon(data=re.age.pandemic, aes(x=t, ymin=exp(`0.025quant`), ymax=exp(`0.975quant`)), alpha=0.2) +
  geom_hline(yintercept = 1)

p3.t.age.inla


re.age.t %>% filter(agey %in%c(35,70)) %>%
 ggplot( aes(x=t, y=exp(mean), group=agey , col=agey))+
  geom_line()+
  theme_classic()+
  #geom_ribbon(data=re.age.pandemic, aes(x=t, ymin=exp(`0.025quant`), ymax=exp(`0.975quant`)), alpha=0.2) +
  geom_hline(yintercept = 1)
...
#pred.lincomb <- inla.out$marginals.lincomb.derived[[1]]

#plot(pred.lincomb[,'x'], pred.lincomb[,'y'])
