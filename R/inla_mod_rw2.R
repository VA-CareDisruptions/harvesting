formula1 <- N_deaths ~  1 + #intercept
  f(agey,model = "rw1",scale.model = TRUE )+  #age-specific intercept
  f(agey2,t,model = "rw1",scale.model = TRUE) +   #age-varying effect for time
  f(agey3,pandemic_period,model = "rw1",scale.model = TRUE)+  #age-varying effect for pandemic
  offset(log.offset)
